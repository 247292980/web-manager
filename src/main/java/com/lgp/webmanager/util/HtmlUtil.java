...
public class HtmlUtil {
    ...
    protected static final org.slf4j.Logger logger = LoggerFactory.getLogger(HtmlUtil.class);

    public static String getImage(String url) {
        String logo = "";
        logo = getPageImg(url);
        if (StringUtils.isBlank(logo)) {
            logo = BASE_PATH + DEFAULT_LOGO;
        }
        return logo;
    }
    ...
    public static String getPageImg(String url) {
        ...
        Document doc;
        try {
            doc = Jsoup.connect(url).userAgent(ConstUtil.USER_AGENT).get();
    ...
    public static Map<String, String> getCollectFromUrl(String url) {
        ...
        try {
            result.put("url", url);
            Document doc = Jsoup.connect(url).userAgent(ConstUtil.USER_AGENT).get();
    ...
    private static Pattern MATCH_CHARSET_PATTERN = Pattern.compile("(?<=charset=)(.+)(?=\")");

    public static String matchCharset(String content) {
        Matcher m = MATCH_CHARSET_PATTERN.matcher(content);
        if (m.find()) {
            return m.group();
        }
        return null;
